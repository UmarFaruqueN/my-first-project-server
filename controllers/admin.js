...
updateUserStatus:async (req,res)=>{
    try {
        ...
        const userData = await User.find({}) 
        return  res.status(200).json({message:"UserUpdated",userData,})
...
updateUser:async (req,res)=>{
    try {
        ...
        if(userEmail) return res.status(400).json({message:"This Email Is Already exist"})
        if(userPhone) return res.status(400).json({message:"This Phone Is Already exist"})

        const updatedUser= await User.findOneAndUpdate({phone:req.body.phone},{$set:{name:req.body.name,email:req.body.email,phone:req.body.phone,active:req.body.active}}) 
...
deleteUser: async (req,res)=>{
    try {
        ...
        if(!deletedUser) return res.status(500).json({message:"didn't deleted user"})
        const userData = await User.find({}) 
