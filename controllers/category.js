...
module.exports = {
    addCategory: async (req, res) => {
        ...
        try {
            ...
            const code = await Category.findOne({categoryCode:req.body.categoryCode})
            const category = await Category.findOne({category:req.body.category})
          
            if(code) return res.status(400).json({message:"This Code Is Already Exist"})
...
updateCategory:async (req,res)=>{
    ...
    try {
        ...
        const catCode = await Category.findOne({categoryCode:req.body.categoryCode})
        const category = await Category.findOne({category:req.body.category})
      
        if(catCode) return res.status(400).json({message:"This Code Is Already Exist"})
        if(category) return res.status(400).json({message:"This Category Is Already Exist"})

        const updatedCategory= await Category.findOneAndUpdate({_id:objectId(req.body._id)},{$set:{categoryCode:req.body.categoryCode,category:req.body.category}}) 
...
deleteCategory: async (req,res)=>{
    try {
        ...
        await Category.remove({categoryCode:req.body.categoryCode})
        const categoryData = await Category.find({}) 
